...
export function isActualMediaUrl(val: any): boolean {
  ...
  if (s.includes(" ") && !s.startsWith("http://") && !s.startsWith("https://")) return false;
  if (/^https?:\/\//i.test(s) || /^data:image\//i.test(s) || /^\/\w+/i.test(s)) return true;
  if (s.includes("drive.google.com") || s.includes("googleusercontent.com") || s.includes("dropbox.com")) return true;
  if (/\.(jpeg|jpg|png|gif|webp|svg|bmp|ico|mp4|webm|mov|ogg)(\?.*)?$/i.test(s)) return true;
  return false;
}
...
export function formatImageUrl(url?: string): string {
  ...
  if (!isActualMediaUrl(clean)) return "";
